...
rutas = []
for line in backend_output.strip().split('\n'):
    match = re.search(r\"\.route\('([^']+)'\", line)
    if match:
        rutas.append(match.group(1))

used = set()
files_to_check = glob.glob('src/crud/*.ts') + glob.glob('src/views/**/*.tsx', recursive=True) + glob.glob('src/views/**/*.ts', recursive=True)
...
for filepath in files_to_check:
    try:
        with codecs.open(filepath, 'r', 'utf-8') as f:
            content = f.read()
            for ruta in rutas:
                base_ruta = re.sub(r'/<[^>]+>', '', ruta)
                if base_ruta in content:
                    used.add(ruta)
    except:
        pass

unused = set(rutas) - used
print('USED:')
for r in sorted(used): print(' -', r)
print('\\nUNUSED:')
